<h1>Modifier l’article "<?= $leArticle->title ?>" (id = <?= $leArticle->id ?>)</h1>
<?php
...
<div style="margin: 20px 0;">
    ...
    <?php foreach ($leArticle->tags as $tag): ?>                     
        <script> document.getElementById('tag-<?= $tag->ArticlesTags['tag_id'] ?>').checked = true;</script>
        <script> document.getElementById('tags[_joinData][<?= $tag->ArticlesTags['tag_id']?>][priority]').value = "<?= $tag->ArticlesTags['priority'] ?>";</script>  
    <?php endforeach; ?>

</div>

<?php
echo $this->Form->button(__("Mettre à jour l’article"));
echo $this->Form->end();
?>
<br/>
<?=
